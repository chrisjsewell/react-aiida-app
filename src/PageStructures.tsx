...
import { AiidaSettingsContext, IAiidaRestNode, getNode, uuidPattern } from './aiidaClient'
...
export function PageStructures(): JSX.Element {
    ...
    return (
        <Grid container spacing={2} className={classes.mainGrid} direction="row-reverse">
            ...
            <Grid item xs={12} sm={12} md={6}>
                <Paper variant="outlined" className={classes.paper}>
                    <div>
                        ...
                        <p>This will eventually be our structure visualiser (coupled with the Optimade Client).</p>
                    </div>
                </Paper>
            </Grid>
            ...
            <Grid item xs={12} sm={12} md={6}>
                <Paper variant="outlined" className={classes.paper}>
                    {/* TODO getting `Uncaught Error: Could not find ID for Fiber "StructureViewer"` with dev tools */}
                    <StructureViewer />
                </Paper>
            </Grid>
        </Grid>
...
export function StructureViewer(): JSX.Element {
    ...
    let data = null as null | IStructureData
    ...
    return (
        <React.Fragment>
            <TextField
                ...
                fullWidth
            />
            {/* The Canvas sets up a Scene and a Camera */}
            <Canvas>
                {/* TODO adding this camera disables controls, maybe also see https://codeworkshop.dev/blog/2020-04-03-adding-orbit-controls-to-react-three-fiber/ */}
                {/* <PerspectiveCamera makeDefault position={[0, 0, 50]} fov={10}> */}
                <OrbitControls enableZoom={true} enableRotate={true} />
                <ambientLight />
                <pointLight position={[10, 10, 10]} />
                <Center alignTop={false}>
                    {data ? <Structure data={data} /> : null}
                </Center>
                {/* </PerspectiveCamera> */}
            </Canvas>
            {data ? <Typography gutterBottom align="right">X: {length(data.attributes.cell[0])}; Y: {length(data.attributes.cell[0])}; Z: {length(data.attributes.cell[0])}</Typography>: null}
            {data ? <StructureTable data={data} /> : null}
        </React.Fragment>
    )
}

function length(point: [number, number, number]){
...
function Structure({ data }: { data: IStructureData }): JSX.Element {
    ...
    return (
        <React.Fragment>
            <Line points={[[0, 0, 0], data.attributes.cell[0]]} color="black" lineWidth={1} dashed={true}/>
            <Line points={[[0, 0, 0], data.attributes.cell[1]]} color="black" lineWidth={1} dashed={true}/>
            <Line points={[[0, 0, 0], data.attributes.cell[2]]} color="black" lineWidth={1} dashed={true}/>
            {
                sites.map((site) => {
                    return <Atom
                        position={site['position'] as Vector3}
                        radius={get(atomData, site['kind_name'], [1])[0]}
                        color={kind2color(site['kind_name'])}
                        opacity={0.9}
                    />
                })
            }
...
function kind2color(kindName: string, asCss: boolean = false) {
    // TODO handle when kindName different to element
    const color = new Color(
        ...get(atomData, kindName, [1, 0, 1, 0]).slice(1, 4)
    )
    if (asCss){
...
function StructureTable({ data }: { data: IStructureData }): JSX.Element {
    ...
    const columns: GridColDef[] = [
        { field: 'kind', headerName: 'Kind', width: 120, renderCell:  (params: GridCellParams) => {
            const color = kind2color(`${params.value}`, true) as string
            return <Avatar 
    ...
    const rows = data.attributes.sites.map((site, index) => {
        return { id: index, kind: site.kind_name, x: site.position[0], y: site.position[0], z: site.position[0] }
    })
    return (
        <div style={{ width: '100%' }}>
